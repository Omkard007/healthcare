...
import { markAlertRead, dismissAlert, markAllAlertsRead, AlertData } from "@/actions/alerts.actions"
...
function getSeverityConfig(severity: string) {
    return severityConfig[severity as AlertSeverity] ?? severityConfig.info
}
    ...
    return (
        <div className="space-y-6 animate-fade-in-up">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                {unread > 0 && (
                    <button onClick={handleMarkAll} className="text-sm text-blue-600 hover:underline font-medium flex items-center gap-1">
            ...
            <div className="space-y-3">
                ...
                {filtered.map(alert => {
                    ...
                    const SeverityIcon = Icon4Severity[alert.severity as AlertSeverity] ?? Info
                    return (
                        <div
                            key={alert.id}
                            className={`relative rounded-2xl border p-4 transition-all cursor-pointer ${cfg.bg} ${cfg.border} ${!alert.read ? "shadow-sm" : "opacity-75"}`}
