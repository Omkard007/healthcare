...
import { User, Scale, Activity, Target, CheckCircle2, ChevronRight } from "lucide-react"
import { upsertHealthProfile } from "@/actions/profile.actions"
import { HealthProfileData } from "@/types"
...
interface Props {
    existingProfile: HealthProfileData | null
}

export function ProfileForm({ existingProfile }: Props) {
    ...
    const [form, setForm] = useState({
        ...
        activityLevel: existingProfile?.activityLevel ?? "light",
        sleepHours: existingProfile?.sleepHours?.toString() ?? "",
        conditions: (existingProfile?.conditions?.length ? existingProfile.conditions : ["none"]) as string[],
        allergies: existingProfile?.allergies?.join(", ") ?? "",
        smoking: existingProfile?.smoking ?? false,
        alcohol: existingProfile?.alcohol ?? false,
        fitnessGoal: existingProfile?.fitnessGoal ?? "maintenance",
    })
    ...
    const handleSave = () => {
        ...
        startTransition(async () => {
            ...
            const result = await upsertHealthProfile({
                ...
                fitnessGoal: form.fitnessGoal as "fat_loss" | "muscle_gain" | "maintenance" | "diabetic_friendly",
            })
        ...
        <div className="max-w-2xl mx-auto space-y-6 animate-fade-in-up">
            ...
            <div>
                ...
                <p className="text-slate-500 text-sm mt-0.5">
                    {existingProfile ? "Update your profile — changes are reflected in your plans instantly." : "Your data is used to generate personalised plans — nothing is shared."}
                </p>
            </div>
                ...
                <div className="flex items-center justify-between mt-6 pt-5 border-t border-slate-100">
                    ...
                    {step < 3 ? (
                        ...
                        <button
                            ...
                            className={`flex items-center gap-2 px-6 py-2.5 text-white text-sm font-bold rounded-xl shadow-md transition-all ${saved ? "bg-emerald-500 shadow-emerald-100" : isPending ? "bg-blue-400 shadow-blue-100 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700 shadow-blue-100"}`}
                        >
                            {saved ? <><CheckCircle2 className="w-4 h-4" /> Saved!</> : isPending ? <>
